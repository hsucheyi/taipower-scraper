...
def extract_pdf_info_from_html(html: str) -> tuple[str, str]:
    ...
    title = title_match.group(1).strip() if title_match else "電價及單位成本結構比較(PDF)"
...
def fetch_html_and_pdf() -> tuple[bytes, str, str]:
    with sync_playwright() as p:
        ...
        try:
            ...
            # 關鍵：不要用 requests，也不要依賴 context.request.get
            # 改用 page.evaluate + fetch + credentials: include
            ...
            if not result["ok"]:
                raise RuntimeError(
                    f"PDF 下載失敗: status={result['status']} {result['statusText']}"
                )

            pdf_bytes = __import__("base64").b64decode(result["base64"])
            if not pdf_bytes:
